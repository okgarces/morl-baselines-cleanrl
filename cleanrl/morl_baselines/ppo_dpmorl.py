...
class Args:
    ...
    env_id: str = "mo-mountaincar-v0" # "CartPole-v1"
...
class Utility_Function_Linear(nn.Module):
    def __init__(self, reward_shape=2, norm=True, lamda=0.1, function_choice=1, keep_scale=True):
        ...
        self.min_val = np.full(reward_shape, -5)
        self.max_val = np.full(reward_shape, 5)
        ...
        weights = simplex_grid(self.reward_shape, 0.25)
        weights.append(np.ones(reward_shape) * 1 / self.reward_shape) # add the barycentre.
...
class ObsInfoWrapper(gym.Wrapper):
    def __init__(self, env):
        ...
        self.actual_reward_dim = self.env.unwrapped.reward_dim
    ...
    def step(self, action):
        ...
        # self.zt = self.zt + reward # Update the zt as zt_next.
        #
...
class MultiEnv_UtilityFunction(mo_gym.wrappers.vector.MOSyncVectorEnv):
    def __init__(
            self,
            venv,
            utility_function,
        ...
        if self.augment_state:
            current_low = self.observation_space.low
            expanded_reward_low = np.full([1, self.reward_dim], -np.inf)
            low = np.hstack([current_low, expanded_reward_low])

            current_high = self.observation_space.high
            expanded_reward_high = np.full([1, self.reward_dim], np.inf)
            high = np.hstack([current_high, expanded_reward_high])
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None) -> tuple[np.ndarray, dict]:
        ...
        if self.augment_state: # Why the in the reset the normalised return?
    ...
    def step(self, action):
        ...
        #                     [info_env['terminal_observation'], normalized_return[index]], 0)
            info['ur'] = self.total_reward.copy() # Utility Return
...
if __name__ == "__main__":
    ...
    utility_functions = {
        'linear': Utility_Function_Linear,
    }
    ...
    # env setup
    utility_function = utility_functions[args.utility_function](3) # TODO
    envs = MultiEnv_UtilityFunction([make_env(args.env_id, i, args.capture_video, run_name) for i in range(args.num_envs)],
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if 'episode' in infos:
                ...
                episode_returns = episode['r']
                # This is for MORL
                for episode_return_env in episode_returns: # Per ENV.
                ...
                episode_lengths = episode['l']
                for episode_length_env in episode_returns:  # Per ENV.
                    for objective, episode_length in enumerate(episode_length_env):
                        writer.add_scalar(f"charts/episodic_length/objective_{objective + 1}", episode_length, global_step)

                episode_utility_returns = infos['ur'] # utility returns
                for utility_return in episode_utility_returns:
