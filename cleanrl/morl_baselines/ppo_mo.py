...
class UtilityFunctionLinear(nn.Module):
    ...
    def set_chosen_utility_function(self, to_choose):
        # Safe. If index to choose is > then reset the chosen function.
        self.function_choice = to_choose % len(self.utility_functions)
...
if __name__ == "__main__":
    ...
    ####################################################################################################################
    for to_train_utility_function in range(args.num_utility_functions):
        # Train first
        utility_function.set_chosen_utility_function(to_train_utility_function)
        ...
        for iteration in range(1, args.num_iterations + 1):
                    ...
                    for index_done, done in enumerate(dones):
                        if done:
                            ...
                            for objective, episode_return in enumerate(episode_return_env):
                                ...
                                writer.add_scalar(f"charts/utility_function_{to_train_utility_function + 1}/episodic_return/objective_{objective + 1}", episode_return, global_step)

                            episode_length_env = episode['l'][index_done]
                            writer.add_scalar(f"charts/utility_function_{to_train_utility_function + 1}/episodic_length", episode_length_env, global_step)

                            utility_return = episode['ur'][index_done] # utility returns
                            writer.add_scalar(f"charts/utility_function_{to_train_utility_function + 1}/utility_returns", utility_return, global_step)

                            utility_return = episode['disc_ur'][index_done]  # utility returns
                            writer.add_scalar(f"charts/utility_function_{to_train_utility_function + 1}/discounted_utility_returns", utility_return, global_step)
            ...
            writer.add_scalar(f"charts/utility_function_{to_train_utility_function + 1}/learning_rate", optimizer.param_groups[0]["lr"], global_step)
            writer.add_scalar(f"losses/utility_function_{to_train_utility_function + 1}/value_loss", v_loss.item(), global_step)
            writer.add_scalar(f"losses/utility_function_{to_train_utility_function + 1}/policy_loss", pg_loss.item(), global_step)
            writer.add_scalar(f"losses/utility_function_{to_train_utility_function + 1}/entropy", entropy_loss.item(), global_step)
            writer.add_scalar(f"losses/utility_function_{to_train_utility_function + 1}/old_approx_kl", old_approx_kl.item(), global_step)
            writer.add_scalar(f"losses/utility_function_{to_train_utility_function + 1}/approx_kl", approx_kl.item(), global_step)
            writer.add_scalar(f"losses/utility_function_{to_train_utility_function + 1}/clipfrac", np.mean(clipfracs), global_step)
